...
ALTER TABLE Arrenda   DROP FOREIGN KEY Arrenda_ibfk_2;
...
EXPLAIN
SELECT A.nif FROM Arrenda A INNER JOIN Fiscaliza F ON A.morada = F.morada AND A.codigo = F.codigo GROUP BY A.nif HAVING COUNT(DISTINCT F.id) = 1;
EXPLAIN
SELECT DISTINCT P.morada, P.codigo_espaco FROM Posto P WHERE (P.morada, P.codigo_espaco) NOT IN ( SELECT P.morada, P.codigo_espaco FROM Posto P NATURAL JOIN Aluga A NATURAL JOIN Estado E WHERE E.estado = 'Aceite');
